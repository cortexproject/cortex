...
func TestIndexStorageFileSystemChunkStorage(t *testing.T) {
	// to keep it simple, use filesystem as chunk storage.
	s, err := e2e.NewScenario(networkName)
	require.NoError(t, err)
	defer s.Close()

	// Start dependencies.
	elasticsearch := e2edb.NewElasticSearch()

	stores := []struct {
		IndexStore, ChunkStore string
	}{{IndexStore: "elastic", ChunkStore: "filesystem"}}
	perStoreDuration := 14 * 24 * time.Hour

	consul := e2edb.NewConsul()
	require.NoError(t, s.StartAndWaitReady(elasticsearch, consul))
	...
	for i := range stores {
		storeConfigs[i] = storeConfig{From: oldestStoreStartTime.Add(time.Duration(i) * perStoreDuration).Format("2006-01-02"),
			IndexStore: stores[i].IndexStore, ObjectStore: stores[i].ChunkStore}
	}

	storageFlags := mergeFlags(ChunksStorageFlags, map[string]string{
		"-experimental.elastic.address":            "http://" + elasticsearch.NetworkHTTPEndpoint(),
		"-experimental.elastic.template-name":      "cortex-template",
		"-experimental.elastic.index-type":         "cortex",
		"-experimental.elastic.number-of-shards":   "1",
		"-experimental.elastic.number-of-replicas": "0",
		"-experimental.elastic.tls-skip-verify":    "true",
		"-local.chunk-directory":                   "/shared", // use mounted volume to let chunks in filesystem be transparent for all components
	})
	...
	for ts := oldestStoreStartTime; ts.Before(now); ts = ts.Add(24 * time.Hour) {
		...
		// Sleep 1s before query to wait push or flush finished. Otherwise the query result will be empty frequently.
...
func TestChunksStorageAllIndexBackends(t *testing.T) {
	...
	stores := []struct {
		IndexStore, ChunkStore string
	}{{IndexStore: "aws-dynamo", ChunkStore: "aws-dynamo"},
		{IndexStore: "bigtable", ChunkStore: "bigtable"},
		{IndexStore: "cassandra", ChunkStore: "cassandra"}}
	perStoreDuration := 14 * 24 * time.Hour
