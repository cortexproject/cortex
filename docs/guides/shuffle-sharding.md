...
1. A mishaving tenant (eg. causing out of memory) could affect all other tenants
...
- A misbehaving tenant will affect only its shard instances. Due to the low overlapping of instances between different tenants, it's probabilistically likely any other tenant will run on different instances or only a subset of instances will match the affected ones.
...
When shuffle sharding is **enabled** setting `-frontend.max-queriers-per-user` (or its respective YAML config option) to a value higher than 0 and lower than the available queriers, the query-frontend will execute each tenant queries across a subset of queriers.
