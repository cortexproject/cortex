...
* [BUGFIX] Querier: Prune deleted tenants from `BucketScanBlocksFinder`'s per-tenant metas maps (`userMetas`/`userMetasLookup`/`userDeletionMarks`) on the partial-error path, matching the existing fetcher eviction. Previously a permanently-failing tenant's scan kept every seen tenant (including deleted ones) alive for the process lifetime and served stale block references. #7728
