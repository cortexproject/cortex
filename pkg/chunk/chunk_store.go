...
func newStore(cfg StoreConfig, schema Schema, index IndexClient, chunks ObjectClient, limits StoreLimits, tombstonesLoader TombstonesLoader) (Store, error) {
...
func (c *store) applyTombstonesToChunks(userID string, chunks []Chunk, queryRangeFrom, queryRangeThrough model.Time) ([]Chunk, error) {
	...
	// ToDo: using query range for now, need to check if we can somehow tell oldest chuns start time and latest chunks end time
