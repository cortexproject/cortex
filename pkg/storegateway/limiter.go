...
func newBytesLimiterFactory(limits *validation.Overrides, userID string, userTokenBucket, instanceTokenBucket *util.TokenBucket, tokenBucketBytesLimiterCfg tsdb.TokenBucketBytesLimiterConfig, getTokensToRetrieve func(tokens uint64, dataType store.StoreDataType) int64, trackerHolder *requestDataBytesTrackerHolder) store.BytesLimiterFactory {
	return func(failedCounter prometheus.Counter) store.BytesLimiter {
		...
		if trackerHolder != nil {
			...
			if reqTracker != nil {
				return newTrackingDataBytesLimiter(innerLimiter, reqTracker)
			}
		}
		return innerLimiter
	}
