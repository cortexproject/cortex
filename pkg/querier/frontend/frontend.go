...
func (f *Frontend) RoundTripGRPC(ctx context.Context, req *httpgrpc.HTTPRequest) (*httpgrpc.HTTPResponse, error) {
	id := int64(0)
	resps := make(chan *ClientToFrontend)
	f.respMtx.Lock()
	f.resps[id] = resps
	f.respMtx.Unlock()

	// Always delete & drain the queue, just in case we got a response on an error path.
	defer func() {
		f.respMtx.Lock()
		delete(f.resps, id)
		select {
	...
	_, err := f.Scheduler.Enqueue(ctx, &EnqueueRequest{
		Id:          id,
		HttpRequest: req,
	})
	if err != nil {
	...
	select {
	case <-ctx.Done():
		// Cancel the query with the schduler, to ensure it doesn't waste querier resources.
		f.Scheduler.Cancel(ctx, &CancelRequest{
			Id: id,
		})
		return nil, ctx.Err()
...
// Response is called by Queriers when they have the result.
func (f *Frontend) Response(ctx context.Context, resp *ClientToFrontend) (*ResponseResponse, error) {
	f.respMtx.Lock()
	defer f.respMtx.Unlock()

	ch, ok := f.resps[resp.Id]
	if !ok {
		return nil, fmt.Errorf("query %d not found", resp.Id)
	}
...
func (f *Frontend) CheckReady(_ context.Context) error {
	...
	connectedClients := f.Scheduler.connectedClients.Load()
