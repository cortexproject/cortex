...
func (s *Scheduler) fragmentAndEnqueueRequest(frontendContext context.Context, frontendAddr string, msg *schedulerpb.FrontendToScheduler, byteLogicalPlan []byte) error {
	...
	lpNode, err := logicalplan.Unmarshal(byteLogicalPlan)
	if err != nil {
		return err
	}
	...
	for _, fragment := range fragments {
		...
		newBody, err := updatePlanInHTTPRequest(fragment)
		if err != nil {
			return err
		}
		...
		err = s.enqueueRequest(frontendContext, frontendAddr, msg, fragment)
