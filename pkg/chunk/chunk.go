...
func (c *Chunk) Decode(decodeContext *DecodeContext, input []byte) error {
	...
	if c.metadataInIndex {
		...
		c.encoded = input
		return errors.Wrap(c.Data.UnmarshalFromBuf(input), "unmarshal old-style")
	}
	...
	if err := binary.Read(r, binary.BigEndian, &metadataLen); err != nil {
		return errors.Wrap(err, "reading metadata len")
	}
	...
	*c = tempMetadata
	...
	if err := binary.Read(r, binary.BigEndian, &dataLen); err != nil {
		return errors.Wrap(err, "reading data len")
	}
