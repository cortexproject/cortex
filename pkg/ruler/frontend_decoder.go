...
type Decoder interface {
	Decode(body []byte) (promql.Vector, Warning, error)
	ContentType() string
}
...
func (p ProtobufDecoder) Decode(body []byte) (promql.Vector, Warning, error) {
	...
	case "scalar":
		...
		return p.scalarToPromQLVector(s), resp.Warnings, nil
	case "vector":
...
func (p ProtobufDecoder) scalarToPromQLVector(s model.Scalar) promql.Vector {
	return promql.Vector{promql.Sample{
		T:      int64(s.Timestamp),
		F:      float64(s.Value),
		Metric: labels.Labels{},
