...
func TestStats_AddQueryInfoFields(t *testing.T) {
	...
	t.Run("add start, end, step, ts, and query fields", func(t *testing.T) {
		...
		stats.AddStep(100)
		stats.AddTs(15000)
		assert.Equal(t, int64(10000), stats.LoadStart())
		assert.Equal(t, int64(20000), stats.LoadEnd())
		assert.Equal(t, int64(100), stats.LoadStep())
		assert.Equal(t, int64(15000), stats.LoadTs())
