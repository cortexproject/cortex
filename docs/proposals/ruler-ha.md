...
With replication enabled, multiple rulers will evaluate the same rule group concurrently.  This will result in duplicated work, leading to duplication of the following:

- recording rule metrics
  - We do not expect this to be a problem, as all replicas must use the same [slotted intervals](https://github.com/prometheus/prometheus/blob/b878527151e6503d24ac5b667b86e8794eb79ff7/rules/manager.go#L509) when evaluating rules, which should result in the same timestamp applying to metrics generated by each replica, with the samples being deduplicated in the ingestion path.
- alerts
- rule group evaluation logging.
- operational metrics

Apart from the increased resources and cost of redundant evaluations, logging and operational metrics, in particular, becomes difficult to monitor and alert on, given that single errors may no longer signal actual failures from the tenants' perspective.

The solution is to replicate only the evaluation trigger, but restrict actual evaluation to only one of the replicas via an elected replica strategy similar to that used for HA sample tracking, using a KV store to coordinate between replicas.
