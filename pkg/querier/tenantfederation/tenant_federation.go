...
type Config struct {
	...
	// RegexCacheSize Maximum number of regex match results cached locally.
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet) {
	f.BoolVar(&cfg.Enabled, "tenant-federation.enabled", false, "If enabled on all Cortex services, queries can be federated across multiple tenants. The tenant IDs involved need to be specified separated by a `|` character in the `X-Scope-OrgID` header.")
	...
	f.IntVar(&cfg.RegexCacheSize, "tenant-federation.regex-cache-size", defaultRegexCacheSize, "[Experimental] Cache size of regex match results used by regex resolver. Each cache entry includes both regex patterns (e.g. `user-.+`) and resolved tenant IDs (e.g. `user-1`). Set to 0 or less to disable caching.")
