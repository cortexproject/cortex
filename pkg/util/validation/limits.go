...
func (l *Limits) RegisterFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&l.RejectOldSamples, "validation.reject-old-samples", false, "Reject old samples at the distributor before forwarding them to ingesters.")
	_ = l.RejectOldSamplesMaxAge.Set("14d")
	f.Var(&l.RejectOldSamplesMaxAge, "validation.reject-old-samples.max-age", "Maximum accepted sample age before rejecting at the distributor.")
	...
	f.Var(&l.OutOfOrderTimeWindow, "ingester.out-of-order-time-window", "[Experimental] Configures the ingester TSDB window for out-of-order samples. Use this to tolerate late samples that are older than the TSDB head minimum time. Disabled (0s) by default.")
