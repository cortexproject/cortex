	...
	tests := map[string]struct {
		...
		"successful scan with all user types": {
			bucketSetup: func(b *bucket.ClientMock) {
				...
				// Deletion marks
				b.MockExists(tenant.GetGlobalDeletionMarkPath("user-1"), true, nil)
				b.MockExists(tenant.GetLocalDeletionMarkPath("user-1"), false, nil)
				b.MockExists(tenant.GetGlobalDeletionMarkPath("user-2"), false, nil)
				b.MockExists(tenant.GetLocalDeletionMarkPath("user-2"), false, nil)
				b.MockExists(tenant.GetGlobalDeletionMarkPath("user-3"), false, nil)
				b.MockExists(tenant.GetLocalDeletionMarkPath("user-3"), false, nil)
			},
	...
	tests := map[string]struct {
		...
		"index read error falls back to base scanner": {
			bucketSetup: func(b *bucket.ClientMock) {
				...
				b.MockIter("", []string{"user-1/"}, nil)
				b.MockIter("__markers__", []string{}, nil)
				b.MockExists(tenant.GetGlobalDeletionMarkPath("user-1"), false, nil)
				b.MockExists(tenant.GetLocalDeletionMarkPath("user-1"), false, nil)
			},
			maxStalePeriod:   1 * time.Hour,
