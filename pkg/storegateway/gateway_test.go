...
func TestStoreGateway_BlocksSharding(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			assert.Equal(t, float64(testData.expectedBlocksLoaded), metrics.GetSumOfGauges("bucket_store_blocks_loaded"))

			// The total number of blocks synced (before filtering) is always equal to the total
			// number of blocks for each instance.
			assert.Equal(t, float64(testData.numGateways*numBlocks), metrics.GetSumOfGauges("blocks_meta_synced"))
		})
	}
}
