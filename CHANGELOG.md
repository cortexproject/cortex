...
* [ENHANCEMENT] Ingester: added following per-ingester limits: max number of series in memory (`-ingester.global-limits.max-series`), max number of users in memory (`-ingester.global-limits.max-users`), max ingestion rate (`-ingester.global-limits.max-ingestion-rate`), and max inflight requests (`-ingester.global-limits.max-inflight-push-requests`). These limits are only used when using blocks storage. Limits can also be configured using runtime-config feature, and current values are exported as `cortex_ingester_global_limit` metric. #3992.
