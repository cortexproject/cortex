...
func (cfg *TSDBConfig) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&cfg.MaxExemplars, "blocks-storage.tsdb.max-exemplars", 0, "Enables experimental support for exemplars in TSDB and sets the maximum number that will be stored. 0 or less means disabled.")
}
