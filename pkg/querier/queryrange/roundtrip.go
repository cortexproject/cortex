...
type Config struct {
	...
	MaxStepAlignment       time.Duration `yaml:"max_step_alignment"`
...
func (cfg *Config) RegisterFlags(f *flag.FlagSet) {
	...
	f.DurationVar(&cfg.MaxStepAlignment, "querier.max-step-alignment", 0, "Queries should not be aligned if step > max-step-alignment, 0 disables it.")
...
func NewTripperware(
	...
	if cfg.AlignQueriesWithStep {
		queryRangeMiddleware = append(queryRangeMiddleware, InstrumentMiddleware("step_align", metrics), StepAlignMiddleware(cfg.MaxStepAlignment.Milliseconds()))
	}
	if cfg.SplitQueriesByInterval != 0 {
