...
* [FEATURE] Querier: Added new `-querier.max-chunk-bytes-per-query` flag. When Cortex is running with blocks storage, the max chunk bytes limit is enforced in the querier and counts the size of chunks returned from ingesters and blocks storage as bytes
