...
type Rule interface {
	...
	// GetSrcTenants gets the source tenants to be used for the rule.
	GetSrcTenants() string
	// GetDestTenant gets the destination teant to store series produced by the rule in.
	GetDestTenant() string
}
...
func (g *Group) Eval(ctx context.Context, ts time.Time) {
	...
	for i, rule := range g.rules {
		...
		func(i int, rule Rule) {
			...
			// Set the OrgID from src tenant for this ctx
			queryCtx := ctx
			srcTenants := rule.GetSrcTenants()
			if srcTenants != "" {
				queryCtx = user.InjectOrgID(ctx, srcTenants)
			}
			vector, err := rule.Eval(queryCtx, ts, g.opts.QueryFunc, g.opts.ExternalURL)
			if err != nil {
			...
			// determine the appender here based on the rule dest tenant, again by ctx injection
			appCtx := ctx
			dst := rule.GetDestTenant()
			if dst != "" {
				appCtx = user.InjectOrgID(ctx, dst)
			}
			// Handle when dest tenant (or owning tenant) is a composite tenant e.g. `a|b|...|z`
			...
			app := g.opts.Appendable.Appender(appCtx)
