...
func (c *BlocksCleaner) cleanUser(ctx context.Context, userID string) error {
	...
	userBucket := backend.NewUserBucketClient(userID, c.bucketClient)
...
func (c *BlocksCleaner) cleanUserPartialBlocks(ctx context.Context, partials map[ulid.ULID]error, userBucket *backend.UserBucketClient, userLogger log.Logger) {
