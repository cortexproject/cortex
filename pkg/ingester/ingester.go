...
func (i *Ingester) Push(ctx context.Context, req *cortexpb.WriteRequest) (*cortexpb.WriteResponse, error) {
	...
	var (
		...
		handleAppendFailure = func(err error, timestampMs int64, lbls []cortexpb.LabelAdapter, copiedLabels labels.Labels, matchedLabelSetLimits []validation.LimitsPerLabelSet, seriesName string) (rollback bool) {
			...
			case errors.Is(cause, storage.ErrOutOfBounds):
				...
				i.validateMetrics.DiscardedSeriesTracker.Track("sample_out_of_bounds", userID, seriesName)
			...
			case errors.Is(cause, storage.ErrOutOfOrderSample):
				...
				i.validateMetrics.DiscardedSeriesTracker.Track("sample_out_of_order", userID, seriesName)
				updateFirstPartial(func() error { return wrappedTSDBIngestErr(err, model.Time(timestampMs), lbls) })

			case errors.Is(cause, storage.ErrDuplicateSampleForTimestamp):
				newValueForTimestampCount++
				i.validateMetrics.DiscardedSeriesTracker.Track("new_value_for_timestamp", userID, seriesName)
				updateFirstPartial(func() error { return wrappedTSDBIngestErr(err, model.Time(timestampMs), lbls) })

			case errors.Is(cause, storage.ErrTooOldSample):
				sampleTooOldCount++
				i.validateMetrics.DiscardedSeriesTracker.Track("sample_too_old", userID, seriesName)
				updateFirstPartial(func() error { return wrappedTSDBIngestErr(err, model.Time(timestampMs), lbls) })

			case errors.Is(cause, errMaxSeriesPerUserLimitExceeded):
				perUserSeriesLimitCount++
				i.validateMetrics.DiscardedSeriesTracker.Track("per_user_series_limit", userID, seriesName)
			...
			case errors.Is(cause, errMaxSeriesPerMetricLimitExceeded):
				...
				i.validateMetrics.DiscardedSeriesTracker.Track("per_metric_series_limit", userID, seriesName)
				updateFirstPartial(func() error {
			...
			case errors.As(cause, &errMaxSeriesPerLabelSetLimitExceeded{}):
				...
				i.validateMetrics.DiscardedSeriesTracker.Track("per_labelset_series_limit", userID, seriesName)
	...
	for _, ts := range req.Timeseries {
		...
		for _, s := range ts.Samples {
			...
			if rollback := handleAppendFailure(err, s.TimestampMs, ts.Labels, copiedLabels, matchedLabelSetLimits, tsLabels.String()); !rollback {
				continue
			}
		...
		if i.limits.EnableNativeHistograms(userID) {
			for _, hp := range ts.Histograms {
				...
				if rollback := handleAppendFailure(err, hp.TimestampMs, ts.Labels, copiedLabels, matchedLabelSetLimits, tsLabels.String()); !rollback {
					continue
				}
				// The error looks an issue on our side, so we should rollback
				if rollbackErr := app.Rollback(); rollbackErr != nil {
