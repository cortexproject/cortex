...
func (l *Limits) RegisterFlags(f *flag.FlagSet) {
	...
	f.IntVar(&l.MaxSeriesPerQuery, "ingester.max-series-per-query", 100000, "The maximum number of series for which a query can fetch samples from each ingesters and block storage. When running in chunk storage this limit is enforced in the ingesters and it's a per-instance limit.. " +
		"When running in block storage mode this limit is enforced on the querier and counts series returned from ingesters and block storage as a per-query limit.")
