...
// ParseFormatted returns the legacy_rulefmt map of a users rules configs. It allows
...
// legacy_rulefmt.RuleGroup sets here either, as only a fully-converted rules.Rule
// is able to track alert states over multiple rule evaluations. The caller
// would otherwise have to ensure to convert the legacy_rulefmt.RuleGroup only exactly
