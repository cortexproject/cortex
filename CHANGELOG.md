...
* [FEATURE] Query Frontend: Add `cortex_query_series` and `cortex_query_bytes` per-user histograms to expose the number of series and bytes returned by queries. These metrics can be enabled with the `-frontend.query-stats-enabled` flag (or its respective YAML config option `query_stats_enabled`). #4343
