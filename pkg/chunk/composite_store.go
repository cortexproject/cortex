...
type TombstonesAnalyzer interface {
	GetDeletedIntervals(labels labels.Labels, from, to model.Time) []model.Interval
	Len() int
...
type Store interface {
	...
	DeleteLabels(ctx context.Context, from, through model.Time, userID string, metric labels.Labels) error
	Stop()
}
