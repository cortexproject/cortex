...
func TestPartitionCompactionPlanner_Plan(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			visitMarkerReadFailed := promauto.With(registerer).NewCounter(prometheus.CounterOpts{
				Name: "cortex_compactor_block_visit_marker_read_failed",
				Help: "Number of block visit marker file failed to be read.",
			})
			visitMarkerWriteFailed := promauto.With(registerer).NewCounter(prometheus.CounterOpts{
