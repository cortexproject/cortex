...
// Copied from https://github.com/jaegertracing/jaeger-client-go/blob/master/utils/rate_limiter.go,
// and added return of wait time.

// RateLimiter is a filter used to check if a message that is worth itemCost units is within the rate limits.
type RateLimiter interface {
	// CheckCredit returns true, if itemCost was subtracted from the balance
	// If balance is not high enough, it returns false, and minimum wait time
	// Wait time can get longer, if other clients use rate limiter too. There is no fairness.
...
func (b *rateLimiter) CheckCredit(itemCost float64) (bool, time.Duration) {
	...
	// not enough credit. let's find out how much time client needs to wait
	missing := itemCost - b.balance
	secondsToWait := missing / b.creditsPerSecond
	return false, time.Duration(float64(time.Second) * secondsToWait)
}
