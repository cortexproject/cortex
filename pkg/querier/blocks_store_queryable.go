...
func NewBlocksStoreQueryableFromConfig(querierCfg Config, gatewayCfg storegateway.Config, storageCfg cortex_tsdb.Config, logger log.Logger, reg prometheus.Registerer) (*BlocksStoreQueryable, error) {
	...
	if gatewayCfg.ShardingEnabled {
		...
		stores, err = newBlocksStoreReplicationSet(storesRing, querierCfg.StoreGatewayGRPCClientConfig, logger, reg)
		if err != nil {
		...
		stores = newBlocksStoreBalancedSet(querierCfg.GetStoreGatewayAddresses(), querierCfg.StoreGatewayGRPCClientConfig, logger, reg)
	}
