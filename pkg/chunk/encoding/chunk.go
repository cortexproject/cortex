...
func (b *Batch) removeValuesForInterval(interval model.Interval) {
...
// deletedChunkIterator keeps index of next deleted interval that we would encounter as we keep moving forward with Scan() and Batch() calls
...
func (dci *deletedChunkIterator) FindAtOrAfter(t model.Time) bool {
	...
	dci.nextDeletedIntervalsIndex = dci.findNextNonDeletedIntervalIndex(t)
	return true
}

func (dci *deletedChunkIterator) findNextNonDeletedIntervalIndex(t model.Time) int {
...
func (dci *deletedChunkIterator) Batch(size int) Batch {
	...
	for dci.nextDeletedIntervalsIndex != -1 && model.Time(batch.Timestamps[batch.Length-1]) >= dci.deletedIntervals[dci.nextDeletedIntervalsIndex].Start {
		batch.removeValuesForInterval(dci.deletedIntervals[dci.nextDeletedIntervalsIndex])
