...
type TableManagerConfig struct {
	...
	DynamoDBPollInterval time.Duration `yaml:"dynamo_db_poll_interval"`

	// duration a table will be created before it is needed.
	CreationGracePeriod time.Duration `yaml:"creation_grace_period"`

	IndexTables ProvisionConfig `yaml:"index_tables"`
	ChunkTables ProvisionConfig `yaml:"chunk_tables"`
}
...
type ProvisionConfig struct {
	...
	InactiveWriteScaleLastN int64             `yaml:"inactive_write_scale_last_n"`
	ReadScale               AutoScalingConfig `yaml:"read_scale"`
	InactiveReadScale       AutoScalingConfig `yaml:"inactive_read_scale"`
	InactiveReadScaleLastN  int64             `yaml:"inactive_read_scale_last_n"`
}
