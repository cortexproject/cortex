...
func (g *PartitionCompactionGrouper) loadExistingPartitionedGroups() (map[uint32]*PartitionedGroupInfo, error) {
	...
	err := g.bkt.Iter(g.ctx, PartitionedGroupDirectory, func(file string) error {
		if !strings.Contains(file, PartitionVisitMarkerDirectory) {
			partitionedGroup, err := ReadPartitionedGroupInfoFile(g.ctx, g.bkt, g.logger, file, g.partitionedGroupInfoReadFailed)
			if err != nil {
				return err
			}
...
func (g *PartitionCompactionGrouper) generatePartitionedGroups(blocks map[ulid.ULID]*metadata.Meta, groups []blocksGroupWithPartition, existingPartitionedGroups map[uint32]*PartitionedGroupInfo, timeRanges []int64) ([]*PartitionedGroupInfo, error) {
	...
	for _, partitionedGroup := range existingPartitionedGroups {
		status := partitionedGroup.getPartitionedGroupStatus(g.ctx, g.bkt, g.partitionVisitMarkerTimeout, g.logger, g.partitionVisitMarkerReadFailed, g.partitionVisitMarkerWriteFailed)
...
func (g *PartitionCompactionGrouper) handleEmptyPartition(partitionedGroupInfo *PartitionedGroupInfo, partition Partition) error {
	...
	visitMarkerManager := NewVisitMarkerManager(g.bkt, g.logger, g.ringLifecyclerID, partitionVisitMarker, g.partitionVisitMarkerReadFailed, g.partitionVisitMarkerWriteFailed)
	visitMarkerManager.MarkCompleted(g.ctx)
...
func (g *PartitionCompactionGrouper) isGroupVisited(partitionID int, visitMarkerManager *VisitMarkerManager) (bool, error) {
	...
	if partitionVisitMarker.IsCompleted() {
		level.Info(g.logger).Log("msg", "partition visit marker with partition ID is completed", partitionVisitMarker.LogInfo())
		return true, nil
	}
	if partitionVisitMarker.IsVisited(g.partitionVisitMarkerTimeout, partitionID) {
		level.Info(g.logger).Log("msg", "visited partition with partition ID", partitionVisitMarker.LogInfo())
		return true, nil
	}
	return false, nil
}
