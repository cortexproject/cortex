...
func (w *Updater) updateBlockMarks(ctx context.Context, old []*BlockDeletionMark) ([]*BlockDeletionMark, map[ulid.ULID]struct{}, int64, error) {
	...
	level.Info(w.logger).Log("msg", "finish iterating markers", "iteration_count", count, "discovered_blocks_count", len(discovered), "duration", time.Since(begin), "duration_ms", time.Since(begin).Milliseconds())
	...
	level.Info(w.logger).Log("msg", "finish getting deleted blocks", "old_blocks_count", len(old), "discovered_blocks_count", len(discovered), "deletion_markers_count", len(out), "duration", time.Since(begin), "duration_ms", time.Since(begin).Milliseconds())
