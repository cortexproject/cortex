...
- When a deletion request is made or cancelled, the cache generation number is incremented for the given tenant to the current timestamp. 
- Inside the query frontend, the cache generation number will be outdated, so the cache is invalidated. 
