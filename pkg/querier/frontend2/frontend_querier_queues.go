...
func (q *queues) getOrAddQueue(userID string, maxQueriers int) chan *schedulerRequest {
	...
	if uq == nil {
		uq = &userQueue{
			ch:    make(chan *schedulerRequest, q.maxUserQueueSize),
...
func (q *queues) getNextQueueForQuerier(lastUserIndex int, querier string) (chan *schedulerRequest, string, int) {
	uid := lastUserIndex
