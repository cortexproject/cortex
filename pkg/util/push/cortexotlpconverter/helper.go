...
type ByLabelName []cortexpb.LabelAdapter
...
func timeSeriesSignature(labels []cortexpb.LabelAdapter) uint64 {
...
func createAttributes(resource pcommon.Resource, attributes pcommon.Map, scope scope, settings Settings,
	ignoreAttrs []string, logOnOverwrite bool, metadata *cortexpb.MetricMetadata, extras ...string,
	...
	labels := make([]cortexpb.LabelAdapter, 0, maxLabelCount)
	// XXX: Should we always drop service namespace/service name/service instance ID from the labels
	// (as they get mapped to other Prometheus labels)?
	attributes.Range(func(key string, value pcommon.Value) bool {
		if !slices.Contains(ignoreAttrs, key) {
			labels = append(labels, cortexpb.LabelAdapter{Name: key, Value: value.AsString()})
		}
		return true
	})
	...
	if settings.EnableTypeAndUnitLabels {
		...
		if metadata.Type != cortexpb.UNKNOWN {
	...
	labels = labels[:0]
	for k, v := range l {
		labels = append(labels, cortexpb.LabelAdapter{Name: k, Value: v})
	}
...
func (c *CortexConverter) addHistogramDataPoints(ctx context.Context, dataPoints pmetric.HistogramDataPointSlice,
...
func getPromExemplars[T exemplarType](ctx context.Context, everyN *everyNTimes, pt T) ([]cortexpb.Exemplar, error) {
	promExemplars := make([]cortexpb.Exemplar, 0, pt.Exemplars().Len())
	for i := 0; i < pt.Exemplars().Len(); i++ {
		...
		promExemplar := cortexpb.Exemplar{
		...
		if traceID := exemplar.TraceID(); !traceID.IsEmpty() {
			...
			promLabel := cortexpb.LabelAdapter{
				Name:  traceIDKey,
				Value: val,
			}
		...
		labelsFromAttributes := make([]cortexpb.LabelAdapter, 0, attrs.Len())
...
func (c *CortexConverter) addSummaryDataPoints(ctx context.Context, dataPoints pmetric.SummaryDataPointSlice, resource pcommon.Resource,
	settings Settings, metadata *cortexpb.MetricMetadata, scope scope,
) error {
	for x := 0; x < dataPoints.Len(); x++ {
		if err := c.everyN.checkContext(ctx); err != nil {
			return err
...
func createLabels(name string, baseLabels []cortexpb.LabelAdapter, extras ...string) []cortexpb.LabelAdapter {
	extraLabelCount := len(extras) / 2
	labels := make([]cortexpb.LabelAdapter, len(baseLabels), len(baseLabels)+extraLabelCount+1) // +1 for name
	copy(labels, baseLabels)

	n := len(extras)
	n -= n % 2
	for extrasIdx := 0; extrasIdx < n; extrasIdx += 2 {
		labels = append(labels, cortexpb.LabelAdapter{Name: extras[extrasIdx], Value: extras[extrasIdx+1]})
	}
...
func addResourceTargetInfo(resource pcommon.Resource, settings Settings, earliestTimestamp, latestTimestamp time.Time, converter *CortexConverter) error {
	...
	labels, err := createAttributes(resource, attributes, scope{}, settings, identifyingAttrs, false, &cortexpb.MetricMetadata{}, model.MetricNameLabel, name)
	if err != nil {
		return err
	}
