...
func (p *PartitionedGroupInfo) getPartitionedGroupStatus(
	...
	for _, partition := range p.Partitions {
		...
		visitMarkerManager := NewVisitMarkerManager(userBucket, userLogger, "PartitionedGroupInfo.getPartitionedGroupStatus", partitionVisitMarker, partitionVisitMarkerReadFailed, partitionVisitMarkerWriteFailed)
		...
		if partitionVisitMarkerExists && (partitionVisitMarker.IsPending() || partitionVisitMarker.IsInProgress()) && !partitionVisitMarker.IsExpired(partitionVisitMarkerTimeout) {
		...
		if partitionVisitMarker.IsFailed() {
			status.PendingPartitions++
		} else {
			status.PendingPartitions++
		}
...
func (p *PartitionedGroupInfo) markAllBlocksForDeletion(ctx context.Context, userBucket objstore.InstrumentedBucket, userLogger log.Logger, blocksMarkedForDeletion *prometheus.CounterVec, userID string) error {
	...
	for _, blockID := range blocks {
		if p.doesBlockExist(ctx, userBucket, userLogger, blockID) && !p.isBlockDeleted(ctx, userBucket, userLogger, blockID) {
			if err := block.MarkForDeletion(ctx, userLogger, userBucket, blockID, "delete block during partitioned group completion check", blocksMarkedForDeletion.WithLabelValues(userID, reasonLabelValue)); err != nil {
			...
			level.Info(userLogger).Log("msg", "marked block for deletion during partitioned group info clean up", "partitioned_group_id", p.PartitionedGroupID, "block", blockID.String())
		}
	}
	return nil
}
