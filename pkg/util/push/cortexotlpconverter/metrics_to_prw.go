...
// CortexConverter converts from OTel write format to Cortex remote write format.
type CortexConverter struct {
	unique    map[uint64]*cortexpb.TimeSeries
	conflicts map[uint64][]*cortexpb.TimeSeries
	everyN    everyNTimes
	metadata  []*cortexpb.MetricMetadata
}

func NewCortexConverter() *CortexConverter {
	return &CortexConverter{
		unique:    map[uint64]*cortexpb.TimeSeries{},
		conflicts: map[uint64][]*cortexpb.TimeSeries{},
	}
}
...
// FromMetrics converts pmetric.Metrics to Cortex remote write format.
func (c *CortexConverter) FromMetrics(ctx context.Context, md pmetric.Metrics, settings Settings) (annots annotations.Annotations, errs error) {
	...
	c.metadata = make([]*cortexpb.MetricMetadata, 0, numMetrics)
...
func isSameMetric(ts *cortexpb.TimeSeries, lbls []cortexpb.LabelAdapter) bool {
...
func (c *CortexConverter) addExemplars(ctx context.Context, dataPoint pmetric.HistogramDataPoint, bucketBounds []bucketBoundsData) error {
...
func (c *CortexConverter) addSample(sample *cortexpb.Sample, lbls []cortexpb.LabelAdapter) *cortexpb.TimeSeries {
...
func (s *PromoteResourceAttributes) promotedAttributes(resourceAttributes pcommon.Map) []cortexpb.LabelAdapter {
	if s == nil {
		return nil
	}

	var promotedAttrs []cortexpb.LabelAdapter
	if s.promoteAll {
		promotedAttrs = make([]cortexpb.LabelAdapter, 0, resourceAttributes.Len())
		resourceAttributes.Range(func(name string, value pcommon.Value) bool {
			if _, exists := s.attrs[name]; !exists {
				promotedAttrs = append(promotedAttrs, cortexpb.LabelAdapter{Name: name, Value: value.AsString()})
			}
			return true
		})
	} else {
		promotedAttrs = make([]cortexpb.LabelAdapter, 0, len(s.attrs))
		resourceAttributes.Range(func(name string, value pcommon.Value) bool {
			if _, exists := s.attrs[name]; exists {
				promotedAttrs = append(promotedAttrs, cortexpb.LabelAdapter{Name: name, Value: value.AsString()})
			}
