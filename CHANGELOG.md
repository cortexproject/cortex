...
* [CHANGE] Querier: it's no more required to set `-frontend.query-stats-enabled=true` in the querier to enable query statistics logging in the query-frontend. The flag is now required to be configured only in the query-frontend and the information will be propagated to the queriers in the internal request. #3595
